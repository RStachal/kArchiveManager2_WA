...
-- ============================================================================
-- LEGACY TOMBSTONE (v1.0) -- arch.usp_PrepWorkBatch_Receiving
-- ----------------------------------------------------------------------------
...
-- ============================================================================
CREATE OR ALTER PROCEDURE [arch].[usp_PrepWorkBatch_Receiving]
    @SourceDb sysname,
    @ArchiveDb sysname,
    @FromUtc datetime2(0) = NULL,
    @ToUtc   datetime2(0) = NULL,
    @MaxDocs int = NULL,
    @RequireAllReceived bit = 0,
    @SafetyLagMinutes int = NULL,
    @WorkBatchId bigint OUTPUT
AS
BEGIN
    SET NOCOUNT ON;
    THROW 50024, 'LEGACY BLOCKED: arch.usp_PrepWorkBatch_Receiving is v1.0 only and has been retired. Use arch.usp_PrepareCandidates (v2.0 prepared-batch model).', 1;
END
GO
