...
| `34_demo_fill_empty_children.sql` | **yes** (WMS rows) | Demo instances only. Seeds the three ORDER children the customer data leaves empty (`t_order_comment`, `t_order_detail_comment`, `t_geek_pick_order`) against orders that already pass the gate, so every configured archive table receives rows. The one script here that writes into the WMS, deliberately and only for a demo |
| `35_expected_volumes.sql` | no | **How many rows the next run will move, per table.** `EXACT` from the prepared keys once PREP has run, `DERIVED` from the gate and cutoff before it — chosen automatically and labelled. Answers what the console's Analysis & Estimates screen does not: that screen sizes the tables, this scopes the run |
